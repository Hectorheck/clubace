...
<body class="d-flex flex-column h-100 @if(Request::is('/') || Request::is('home')) bg-login @endif">
	<header class="bg-anaranjado" style="background-color:{{--$agenda->recintos->clubes->color_2 --}}green!important">
		<div class="degradado">
	...
	<div class="mt-auto pt-0">
		...
		<div class="bloque-footer pd-cuadro box-cuadro pt-25">
			<div class="container">
				...
				<form action={{  $transaction->return_url }}>
					@csrf
					@method('PUT')
					...
					<div class="row">
						<div class="col-12">
							<table class="table">
								<thead>
									{{-- @dd($agenda) --}}
									{{-- Illuminate\Support\Collection Funciona tambien en este caso y otros MAS --}}
									<tr>
										<th scope="col">Código Transacción:</th>
										<td>{{ ($agenda instanceof \Illuminate\Database\Eloquent\Collection) ? $transaction->agendas_id : $agenda['id'] }}</td>
									</tr>
								</thead>
								<tbody>
									<tr>
										<th scope="row">Código Reserva</th>
										<td>{{ ($agenda instanceof \Illuminate\Database\Eloquent\Collection) ? $agenda->first()->reserva_id : $agenda['reserva_id'] }}</td>
									</tr>
									<tr>
										<th scope="row">Valor</th>
										<td>$ {{ number_format($transaction->amount, 0, ',', '.') }}</td>
									</tr>
									@if($agenda instanceof \Illuminate\Database\Eloquent\Collection)
									<tr>
										<td colspan="2">Bloques agendados</td>
									</tr>
									@foreach($agenda as $val)
										...
										<tr>
											...
											<td>{{ $val->hora_inicio_bloque }}</td>
										</tr>
										<tr>
											<th scope="row">Hora Termino</th>
											<td>{{ $val->hora_fin_bloque }}</td>
										</tr>
										<tr>
											<th scope="row">Recinto</th>
											<td>{{ $val->recintos->nombre }}</td>
										</tr>
									@endforeach
									@else
									<tr>
						...
						<div class="col-12">
							<div class="col">
								{{-- <a href="#" data-toggle="modal" data-target="#modalPay" class="btn btn-success mb-25 mt-25">Continuar Pago</a> --}}
								<button type="submit" class="btn btn-success mb-25 mt-25">Confirmar</button>
							</div>
						</div>
					</div>
					{{-- MODAL --}}
